...
function Dashboard() {
  ...
  useEffect(() => { obtenerMeses() }, [])
  useEffect(() => { if (mesActivo) calcularTotales() }, [mesActivo])
  ...
  async function obtenerMeses() {
    ...
    setMeses(data); setCargando(false)
  }
  ...
  async function calcularTotales() {
    try {
      ...
      const sueldosTrabajadores = (trabajadoresRes.data || []).reduce((sum, t) => sum + parseFloat(t.monto || 0), 0)
  ...
  async function handleCrearMes(e) {
    ...
    if (error) { setError(error.message); setCargando(false); return }
    setMesActivo(data); setMostrarForm(false); setNombreMes(''); setMes(''); obtenerMeses(); setCargando(false)
  }
    ...
    <div className="flex min-h-screen bg-[#F8F9FA] text-[#344054] font-sans overflow-hidden">
      ...
      `}>
        <div className="p-6 flex justify-between items-center border-b border-gray-100">
          ...
          <button className="md:hidden p-2" onClick={() => setMenuMovilAbierto(false)}>
            <X size={20} />
          </button>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          <button 
            onClick={() => { setMesActivo(null); setMenuMovilAbierto(false); }}
            className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-xl transition-colors ${!mesActivo ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50'}`}
          >
          ...
          {mesActivo && (
            <div className="pt-4 space-y-1">
              <p className="px-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Módulos</p>
              {[
      ...
      <main className="flex-1 overflow-y-auto h-screen scroll-smooth">
        ...
        <header className="bg-white/80 backdrop-blur-md sticky top-0 z-30 border-b border-gray-100 px-4 md:px-8 py-3 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-sm font-medium text-gray-400 hidden sm:block">Jueves, 20 de Febrero</h2>
          </div>
          <div className="flex items-center gap-3">
             <span className="text-sm font-medium hidden sm:block">Hola, {usuario.user_metadata.nombre}</span>
          ...
          {!mesActivo && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                <div>
                  <h3 className="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight leading-tight">¡Buen día!, {usuario.user_metadata.nombre}</h3>
                  <p className="text-gray-500 mt-1">Selecciona o crea un periodo para tus métricas.</p>
                </div>
                <button 
                  onClick={() => setMostrarForm(true)}
                  className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-full text-sm font-bold shadow-lg shadow-blue-100 transition-all flex items-center justify-center gap-2"
                >
                  + Nuevo Proyecto
                </button>
              </div>
              ...
              <div className="bg-white rounded-2xl md:rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="px-5 md:px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                  <h4 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                    Mis Meses <span className="text-xs font-medium text-gray-400 bg-white px-2 py-0.5 rounded-md border border-gray-200">{meses.length}</span>
                  </h4>
                </div>
                <div className="divide-y divide-gray-100">
                  {meses.map((m) => (
                    <div key={m.id} className="px-5 md:px-6 py-4 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-gray-700 truncate max-w-[150px] sm:max-w-none">{m.nombre}</span>
                      </div>
                      <button 
                        onClick={() => setMesActivo(m)}
                        className="px-4 py-2 rounded-lg bg-blue-600 text-white text-xs font-bold hover:bg-blue-700 transition-all shadow-sm"
                      >
                        Ver detalles
                      </button>
                    </div>
          ...
          {mesActivo && (
            <div className="space-y-6 md:space-y-8">
              <div className="flex flex-col sm:flex-row justify-between items-start gap-3">
                <h3 className="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight">{mesActivo.nombre}</h3>
                <button onClick={() => setMesActivo(null)} className="w-full sm:w-auto px-4 py-2 border border-gray-200 rounded-xl text-xs font-bold bg-white hover:bg-gray-50 transition-all shadow-sm">
                  Cambiar Mes
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                  <p className="text-blue-600 text-xs font-bold uppercase tracking-wider">Facturado</p>
                  <p className="text-2xl font-bold text-gray-900 mt-1">{totalFacturas.toFixed(2)} €</p>
                ...
                {/* Estimado */}
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                  <p className="text-purple-600 text-xs font-bold uppercase tracking-wider">Estimado próximo</p>
                  <p className="text-2xl font-bold text-gray-900 mt-1">{totalRutas.toFixed(2)} €</p>
                ...
                {/* Gastos */}
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                  <p className="text-orange-600 text-xs font-bold uppercase tracking-wider">Gastos totales</p>
                  <p className="text-2xl font-bold text-gray-900 mt-1">{totalGastos.toFixed(2)} €</p>
                ...
                {/* Ganancia real */}
                <div className={`p-5 rounded-2xl border shadow-sm ${esPositivo ? 'bg-white border-green-200' : 'bg-red-50 border-red-200'}`}>
                  <p className={`text-xs font-bold uppercase tracking-wider ${esPositivo ? 'text-green-600' : 'text-red-600'}`}>Ganancia real</p>
                  <p className={`text-2xl font-bold mt-1 ${esPositivo ? 'text-green-600' : 'text-red-600'}`}>
                    {gananciaReal.toFixed(2)} €
                  </p>
                  <div className="mt-3">
                    <span className={`inline-block text-[10px] font-bold px-2 py-1 rounded-md ${esPositivo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-6">
              <div className="flex justify-between items-center mb-5">
                <h3 className="text-xl font-bold text-gray-900">Nuevo periodo</h3>
              ...
              <form onSubmit={handleCrearMes} className="space-y-4">
                <div>
                  ...
                  <input type="text" placeholder="Ej: Febrero 2025" value={nombreMes} onChange={(e) => setNombreMes(e.target.value)} required
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Mes</label>
                  ...
                  <div>
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Año</label>
                    <input type="number" value={anio} onChange={(e) => setAnio(e.target.value)} required
                      className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                  </div>
                </div>
                <div className="pt-2">
                  <button type="submit" disabled={cargando} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-all">
